...
- `ChineseLocalization-v<模组版本>.zip`：独立简体中文模组，解压到游戏的 `Mods` 目录。
- `ContentUnlocker-v<模组版本>.zip`：独立隐藏内容解锁模组，解压到游戏的 `Mods` 目录。
...
未压缩的四个发布目录统一写入 `dist/unpacked/`，四个最终 ZIP 直接写入 `dist/`
